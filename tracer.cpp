...
Vec trace(const Vec &originalRay, Vec &rayDirection, vector<Sphere> &spheres, const int &currentDepth){
    ...
    Vec surfaceColor = 0;
    Vec intersectionLocation = rayDirection * near + originalRay; //calculating the intersection
...
void render(vector<Sphere> &spheres){
    ...
    float angle = tan(PI*0.5*(fieldOfView / 180));
    ...
    ofstream ofs("./untitled.ppm", ios::out | ios::binary); //rendering the image to output file -> untitled.ppm
    ofs << "P6\n" << width << " " << height << "\n255\n";
    for( int i = 0; i < width*height; i++ ){
        ofs << (char)(min(float(1), image[i].x)*255) << (char)(min(float(1), image[i].y)*255) << (char)(min(float(1), image[i].z)*255);
    }
    ofs.close();
    delete[] image;
}
...
int main(int argc, char **argv){
    ...
    // values: center -> surface color -> emission left out -> radius -> transparency -> reflection
    Vec c(0.0, -10004, -20);
    Vec s(0.20, 0.20, 0.20);
    Vec e(0);
    float one = 10000;
    float two = 0;
    float three = 0;
    spheres.push_back(Sphere(c, s, e, one, two, three));
